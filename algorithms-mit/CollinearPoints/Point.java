...
public class Point implements Comparable<Point> {
    ...
    public int compareTo(Point that) {
        int val = 0;
        if(this.y > that.y) val = 1;
        else if(this.y < that.y) val = -1;
        else {
            if(this.x > that.x) val = 1;
            else if(this.x < that.x) val = -1;
        }
        return val;
    }
     ...
     * its two argument points by the slopes they make with the invoking point (x0, y0). 
